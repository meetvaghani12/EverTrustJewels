/**
 * Converts raw necklace/pendant API data → Product objects, downloads images, merges into products.ts
 * Run: node scripts/build-necklaces.mjs
 */
...
const RAW_FILE    = join(process.cwd(), "scripts/output/api-necklaces-raw.json");
...
console.log(`Unique necklace/pendant products from API: ${unique.length}`);
...
for (const p of unique) {
  ...
  const safe = style.replace(/[^a-zA-Z0-9\-]/g, "") || `necklace-${idCounter}`;
  ...
  const subCat = (p.SubCatName || "pendant").trim();

  // Map category — pendants go to "pendant", necklaces to "pendant" (matches ProductCategory type)
  const catName = (p.CatName || "").trim().toLowerCase();
...
writeFileSync(join(process.cwd(), "scripts/output/necklaces-final.json"), JSON.stringify(newProducts, null, 2));
console.log(`\n✅ ${newProducts.length} new necklace/pendant products ready`);
...
console.log(`✅ Merged ${newProducts.length} necklaces/pendants into src/data/products.ts`);
