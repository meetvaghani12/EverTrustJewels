/**
 * Converts raw earring API data → Product objects, downloads images, merges into products.ts
 * Run: node scripts/build-earrings.mjs
 */
...
const RAW_FILE    = join(process.cwd(), "scripts/output/api-earrings-raw.json");
...
console.log(`Unique earring products from API: ${unique.length}`);
...
for (const p of unique) {
  ...
  const safe = style.replace(/[^a-zA-Z0-9\-]/g, "") || `earring-${idCounter}`;
  ...
  const subCat = (p.SubCatName || "earrings").trim();
...
writeFileSync(join(process.cwd(), "scripts/output/earrings-final.json"), JSON.stringify(newProducts, null, 2));
console.log(`\n✅ ${newProducts.length} new earring products ready`);
...
console.log(`✅ Merged ${newProducts.length} earrings into src/data/products.ts`);
