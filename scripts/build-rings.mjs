/**
 * Converts raw API data → Product objects, downloads images, merges into products.ts
 * Run: node scripts/build-rings.mjs
 */
...
const RAW_FILE    = join(process.cwd(), "scripts/output/api-products-raw.json");
const PRODUCTS_TS = join(process.cwd(), "src/data/products.ts");
const IMAGES_DIR  = join(process.cwd(), "public/images/products");
const OUT_JSON    = join(process.cwd(), "scripts/output/rings-final.json");
...
console.log(`Unique products from API: ${unique.length}`);

// Load existing style numbers to skip
...
for (const p of unique) {
  ...
  // Derive safe filename from style number
  const safe = style.replace(/[^a-zA-Z0-9\-]/g, "") || `ring-${idCounter}`;

  // Download white image
  const whiteExt = (imgWhiteUrl.split(".").pop()?.split("?")[0] || "jpg").substring(0, 4);
  const whiteFile = `${safe}-white.${whiteExt}`;
  const whitePath = join(IMAGES_DIR, whiteFile);
  const whiteOk = await downloadImage(imgWhiteUrl, whitePath);

  // Download yellow image
...
// Save final JSON
writeFileSync(OUT_JSON, JSON.stringify(newProducts, null, 2));
console.log(`\n✅ ${newProducts.length} new ring products ready`);
...
console.log(`✅ Merged ${newProducts.length} rings into src/data/products.ts`);
